"""
🐱 Single Class Training Demo - CAT Only
Train a node ONLY on CAT images, then test predictions on CAT and DOG images.
...
def visualize_predictions(cat_results, dog_results, save_path='results/plots/single_class_predictions.png'):
    """Visualize sample predictions"""
    fig, axes = plt.subplots(4, 5, figsize=(15, 12))
    fig.suptitle('🐱🐕 Predictions: Node Trained ONLY on CAT', fontsize=16, fontweight='bold')
    ...
    # Second row: CAT probability bars
    for i in range(5):
        ax = axes[1, i]
    ...
    # Third row: DOG images (5 samples)
    for i in range(5):
        ax = axes[2, i]
    ...
    # Fourth row: DOG probability bars
    for i in range(5):
        ax = axes[3, i]
...
def main():
    print_header("🐱 SINGLE CLASS TRAINING DEMO - CAT ONLY")
    ...
    # Partition data - only CAT class
    print_section("🔀 Data Partitioning - CAT ONLY")
    ...
    # Use class-based partitioning to get only CAT
    cat_subset = partitioner.partition_class_based([[CAT_CLASS]])[0]
    
    print(f"  Node data: {len(cat_subset)} samples (only CAT class)")
    ...
    # Training
    print_section("🚀 Training on CAT Images Only")
    num_epochs = 10
    ...
    print_section("📊 AFTER Training on CAT")
    ...
    print(f"\n  🐱 CAT Images (True class: CAT):")
    print(f"     Avg probability for CAT class:")
    print(f"       Before training: {cat_prob_before:.4f}")
    print(f"       After training:  {cat_prob_after:.4f}")
    print(f"       Improvement:     +{cat_prob_after - cat_prob_before:.4f}")
    
    print(f"\n  🐕 DOG Images (True class: DOG, but model never saw DOG):")
    print(f"     Avg probability for DOG class:")
    print(f"       Before training: {dog_prob_before:.4f}")
    print(f"       After training:  {dog_prob_after:.4f}")
    print(f"       Change:          {dog_prob_after - dog_prob_before:+.4f}")
    
    # What does model predict for DOG images?
    dog_preds_after = dog_results_after['predictions']
    unique, counts = np.unique(dog_preds_after, return_counts=True)
    
    print(f"\n  🤔 What does model predict for DOG images?")
    for class_id, count in sorted(zip(unique, counts), key=lambda x: x[1], reverse=True)[:5]:
        percentage = 100 * count / len(dog_preds_after)
        print(f"     {CIFAR10_CLASSES[class_id]:12s}: {count:3d} times ({percentage:.1f}%)")
    
    # Visualize
    print_section("🎨 Creating Visualization")
    visualize_predictions(cat_results_after, dog_results_after)
    
    print_header("✅ Demo Completed!")
    
    print("\n  🎯 Key Findings:")
    print(f"  • Model trained ONLY on CAT images ({len(cat_subset)} samples)")
    print(f"  • On CAT images: probability increased from {cat_prob_before:.4f} to {cat_prob_after:.4f}")
    print(f"  • On DOG images: model has NOT learned to recognize DOG")
    print(f"  • Model often confuses DOG with other classes (never saw DOG during training)")
